"""AI Edit diff overlay paints, in colour, under the X11/Xft backend.
...
(covered by the pyte tests) versus Xlib colours here.  This drives a real xwpe
under the headless X server, runs an AI edit whose mock reply rewrites the file,
and asserts the overlay both painted and added red AND green regions the editor
did not have before.  Self-skips if the X11 harness or --enable-ai is missing.
"""
import subprocess
import pytest
...
# A rewrite that differs from the fixture's multi-line t.c, so the diff carries
# both removed (red) and added (green) lines.
...
def _red_green(img):
    """(red_px, green_px) sampled across the image -- red = high R, low G/B;
    green = high G, low R/B.  Every 2nd pixel is enough and keeps it quick."""
    px = img.load()
    w, h = img.size
    red = green = 0
...
def test_ai_diff_overlay_colours_under_x11(xwpe):
    ...
    assert changed_pixels(before, after) > 2000, \
        "the diff overlay did not paint under X11/Xft"
